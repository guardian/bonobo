...
/**
 * Copied from Play to workaround issue with compile time DI.
...
object CSRFAction {
  private val HeaderNoCheck = "nocheck"

  private val filterLogger = play.api.Logger("play.filters")
  /**
  ...
  private[csrf] def checkCsrfBypass(request: RequestHeader, config: CSRFConfig): Boolean = {
    if (config.headerBypass) {
      if (request.headers.get(config.headerName).exists(_ == HeaderNoCheck)) {
        ...
        filterLogger.trace("[CSRF] Bypassing check because " + config.headerName + ": " + HeaderNoCheck + " header found")
        true
  ...
  private[csrf] def clearTokenIfInvalid(request: RequestHeader, config: CSRFConfig, errorHandler: ErrorHandler, msg: String): Future[Result] = {
    ...
    errorHandler.handle(request, msg) map { result =>
      CSRFSupport.getToken(request).fold(
